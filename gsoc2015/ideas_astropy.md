...
### Description
The astropython.org site is one of the top two generic informational / resource sites about Python in astronomy.  This site uses Google App Engine and is basically all custom code built around the bloggart engine.  Currently it is getting a bit stale for a few reasons:
...
The proposal is to start over with all modern tools to bring fresh energy and involvement into astropython.org.  All details of how to do this to be determined, but one requirement is to migrate all the current content.  Part of this would be re-evaluating current resources as well as digging around to freshen up the resource list.
...
### Description
Now that Astropy can transform from horizontal to sky coordinates it’s possible to develop tools for observation planning / scheduling, see the nice example by Erik. It might be nice to start developing an affiliated package that can be used by observers and observatories to plan and schedule observations. This project could go in a few different directions, including (1) creating typical tables and plots for observation planning (2) optimising scheduling of observations for given target lists and telescope slew speed / exposure lengths for a given night or even month / year (3) contribute sun / moon rise / set functionality to astropy coordinates (4) a desktop or web GUI, …
The project could start with a look at the functionality of existing tools and then gather some input on the astropy mailing list what the community wants.
The student should have an interest in coordinates, observations planning / scheduling and plotting / GUIs.
...
Currently Astropy models have an evaluate function that can be used to evaluate them on a grid of pixel centers, there’s also an oversampling function to get a better representation of the expected flux in pixels.
It would be useful to add methods that allow fast and precise rasterization of models, similar to what graphics libraries do (sparse subsampling or resampling of models evaluated on grids that are appropriate for each model, or anti-aliasing).
If someone is interested in working on this they should talk to use soon, there’s different options what to do, e.g. possibly add optional extension, sampling grid and bounding box information to the Astropy model classes, or contribute rasterisation code to astropy.modeling or photutils, or expand the existing resampling code in https://github.com/astrofrog/reproject/ 
The student should be interested in model fitting and image rasterisation as well as profiling and extensive testing of a given method to make it “just work” for the end user.
...
### Description
The pyregion package is very useful to work with ds9 and CIAO region files. It is now at https://github.com/astropy/pyregion but it is unfinished … someone has to improve and polish it. Specifically the region file parser is very slow (see https://github.com/astropy/pyregion/issues/48) and someone interested in parsing should find out why and make it fast. There’s several other things to do, e.g. using astropy coordinates everywhere and implementing tests that it’s compatible with ds9 at high precision.
If this is not enough for a GSoC project, this could be extended with Python functions to read / write / visualise MOC files (http://www.ivoa.net/documents/MOC/) or to unify and improve the existing Python interfaces to ds9 (https://github.com/ericmandel/pyds9/issues/2 ).
The student should be interested in sky coordinates and regions, parsing, visualisation, writing tests and docs, and for the ds9 interfaces some Cython coding is probably needed.